...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # classifier loss.                                                          #
    #############################################################################
    #computing the loss. doing it exponentially
    scores_exp = np.exp(scores - np.max(scores, axis=1, keepdims=True))

    sum = np.sum(scores_exp,axis=1,keepdims=True)
    probs = scores_exp/sum

    indices = [range(N),y]
    correct_class_score = probs[indices]
    ...
    grads['W2'] = h1.T.dot(probs)
    grads['b2'] = np.sum(probs, axis=0, keepdims=True)
  ...
  def predict(self, X):
    ...
    #activation function sigmoid
    temp = np.maximum(0,(X.dot(W1) + b1)).dot(W2) + b2
